...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood$Movie[which(bollywood$Tcollection==(sort(bollywood$Tcollection,partial=n-1)[n-1]))]
...
# Store the names of 4 movies in same sequence in movie_result vector
  ...
  movie_result <- as.vector(sapply(bollywood[,4:7],function(x) bollywood$Movie[which(x==max(x,na.rm = T))]))
